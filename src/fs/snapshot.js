...
const snapshot = async () => {
  ...
  try {
      ...
      for (const element of elements) {
        if (element.isFile()) {
          ...
          snapshots.entries.push({
            path: join(element.parentPath.replace(snapshots.rootPath, ""),element.name).replaceAll('\\', '/'),
            type: "file",
            size: fileStats.size,
    ...
    await writeFile(join(BASE_PATH, "snapshot.json"), JSON.stringify(snapshotObjectResult, null, 2));
  } catch (error) {
